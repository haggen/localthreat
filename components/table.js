...
export default class Table extends Component {
  ...
  compareDanger = (a, b) => {
    return a.danger == b.danger
      ? 0
      : (a.danger < b.danger ? -1 : 1) * this.state.sorting.dir;
  };

  compareGangs = (a, b) => {
    return a.gangs == b.gangs
      ? 0
      : (a.gangs < b.gangs ? -1 : 1) * this.state.sorting.dir;
  };

  compareKills = (a, b) => {
    return a.kills == b.kills
      ? 0
      : (a.kills < b.kills ? -1 : 1) * this.state.sorting.dir;
  };

  compareLosses = (a, b) => {
    return a.losses == b.losses
      ? 0
      : (a.losses < b.losses ? -1 : 1) * this.state.sorting.dir;
  };
  ...
  handleSort = key => {
    const dir =
      key === this.state.sorting.key ? this.state.sorting.dir * -1 : -1;
    let func;
    switch (key) {
      case "char":
  ...
  render({}, { rows, sorting }) {
    const sortedRows = rows.sort(sorting.func).map(row => {
      return <Row key={row.char.name} {...row} />;
    });
