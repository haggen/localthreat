...
export default class App extends Component {
  ...
  handlePaste = e => {
    ...
    if (contents.length > 500) {
      contents.splice(500);
      notification = (
        <p>You pasted {contents.length - 500} names over the limit of 500.</p>
      );
    }
  ...
  componentDidMount() {
    if (location.pathname != "/") {
      this.setState({ loading: true });
      PasteRepository.fetch(location.pathname.substr(1))
        .then(paste => {
          this.setState({ paste, loading: false });
        })
        .catch(_ => {
          this.setState({
            notification: <p>Couldn't find the scan you requested.</p>,
            loading: false
          });
        });
    } else {
  ...
  render({}, { notification, paste, loading }) {
    return (
      <main
        class={`main${loading ? " main--loading" : ""}`}
      ...
      >
        ...
        <footer class="footer">
          <p class="footer__legal-1">
            &copy; 2017 Arthur Corenzan &middot;{" "}
            <a
              href="https://github.com/haggen/localthreat"
              target="blank"
              rel="noopener noreferrer"
            >
              <span class="plate">GitHub</span>
            </a>{" "}
            &middot; Data provided by{" "}
            <a
              href="https://esi.evetech.net/latest/"
              target="blank"
              rel="noopener noreferrer"
            >
              <span class="plate">ESI</span>
            </a>{" "}
            and{" "}
            <a
              ...
              rel="noopener noreferrer"
            >
              <span class="plate">zKillboard</span>
            </a>{" "}
            &middot; Tips go to{" "}
            <a
              ...
              rel="noopener noreferrer"
            >
              <span class="plate">Jason Chorant</span>
            </a>
          </p>
